...
> recommended default: present it as an open yes/no.** Only after they answer do
...
- **Run a tight scoping interview before building.** Present the axes using the environment's
  ...
  **Always ask (every build — do not skip, do not collapse):**
  ...
  - **AI chat / Q&A interface?** — don't assume no. A built-in "ask a question" chatbot
    ...
    If yes, ask which providers (Claude / OpenAI / Gemini) and which of the two paths. **Ask this as
    a genuine yes/no choice — do NOT pre-select or recommend "no," and don't quietly assume it.**
    Only if the user truly doesn't engage, proceed without chat (designed so it can be added later)
    and say so.

  **Default unless the topic or their answers make it relevant (state your default; only ask if it matters):**
  - **Depth / length** — *quick one-pager* (~3 min, one core idea, ~1 demo) · **standard** (~8–10
    min, the default) · *comprehensive deep-dive* (longer, every facet, more sections + demos). This
    sets how many sections and micro-demos you build and how much lands in the "go deeper" layers vs
    the surface. Default **standard**; offer the toggle when the user signals they want a fast skim
    or an exhaustive guide. (Distinct from *audience/depth*, which is who it's for.)
...
- **`review-mode.js` — Review & edit overlay.** Front end of the Phase 6 revise loop. **Inline it
  by DEFAULT in every single-file explainer** (paste before `</body>` + add `<body
  data-review-toggle>`); only omit if the user explicitly opts out. Gives reviewers Preview / Edit
  text / Add note; **Download edits** exports a
